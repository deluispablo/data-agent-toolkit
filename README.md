...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="agents/csv_inspector/docs/assets/hero-dark.svg">
  <img alt="csv-inspector samples the head and tail of a messy CSV export, asks a local LLM, and returns its encoding, delimiter, header row, footer lines and column names" src="agents/csv_inspector/docs/assets/hero-light.svg" width="900">
</picture>
</a>
...
```bash
pip install "csv-inspector @ git+https://github.com/deluispablo/data-agent-toolkit@csv-inspector-v0.7.0#subdirectory=agents/csv_inspector"
```
...
|---|---|---|
| `csv_inspector_api` | A FastAPI service that embeds `csv-inspector`: inspect an upload (`POST /inspect`), a streamed body (`POST /inspect/raw`) or a Cloud Storage object read with ranged requests only (`POST /inspect/gcs`, optional `[gcs]` extra). | `cd examples/csv_inspector_api && uv run uvicorn --app-dir src csv_inspector_api.app:create_app --factory` · [README](examples/csv_inspector_api/README.md) |
