...
| `_inspect.py` | `inspect_csv` / `ainspect_csv`: a `_Run` resolves models and settings (before the source is read), then, after sampling, runs the prompt through the primary and fallback models within one time budget (`_Deadline`, `PRIMARY_SHARE` for a model followed by another), tallies their usage (`_UsageTally`) and grounds the answer. `run` and `arun` differ only in one `await`. |
