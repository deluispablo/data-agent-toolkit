...
A file's head and tail windows (``_PathReader``) are tested not only for
correctness but also, via a ``Path.open()`` spy, for *how* they are read: they
...
from csv_inspector._sampling import _PathReader, sample_source
...
def test_path_head_respects_byte_limit(tmp_path: Path) -> None:
    """Only the requested number of leading bytes should be read."""
    target = tmp_path / "large.csv"
    target.write_bytes(b"a,b,c\n" * 1000)

    sample = _PathReader(target).head(10)

    assert sample == b"a,b,c\na,b,"
    assert len(sample) == 10


def test_path_head_missing_file_raises_domain_error(tmp_path: Path) -> None:
    """A missing source file should raise the domain-specific exception."""
    missing = tmp_path / "does_not_exist.csv"

    with pytest.raises(FileSampleReadError):
        _PathReader(missing).head(4096)


def test_path_head_reads_real_sample_fixture() -> None:
    """The bundled sample.csv fixture should be readable end-to-end."""
    sample = _PathReader(SAMPLE_CSV_PATH).head(4096)

    assert b"Fecha;Cliente" in sample


def test_path_head_empty_file_returns_empty_bytes(tmp_path: Path) -> None:
    """An empty file should yield an empty sample, not an error."""
    target = tmp_path / "empty.csv"
    target.write_bytes(b"")

    assert _PathReader(target).head(4096) == b""


def test_path_head_never_reads_the_whole_file(
    ...
    head = _PathReader(target).head(len(marker))

    assert head == marker
    assert read_calls == [len(marker)]


def test_path_tail_respects_byte_limit(tmp_path: Path) -> None:
    """Only the requested number of trailing bytes should be read."""
    target = tmp_path / "large.csv"
    target.write_bytes(b"a,b,c\n" * 1000)

    tail = _PathReader(target).tail(0, 10, 1)

    assert tail == (b"a,b,c\n" * 1000)[-10:]
    assert len(tail) == 10


def test_path_tail_smaller_than_limit_returns_whole_file(tmp_path: Path) -> None:
    """A file smaller than ``n_bytes`` should be returned in full."""
    target = tmp_path / "small.csv"
    target.write_bytes(b"a,b,c\n1,2,3\n")

    tail = _PathReader(target).tail(0, 4096, 1)

    assert tail == b"a,b,c\n1,2,3\n"


def test_path_tail_exact_file_size(tmp_path: Path) -> None:
    """A file exactly ``n_bytes`` long should be returned in full."""
    content = b"0123456789"
    target = tmp_path / "exact.csv"
    target.write_bytes(content)

    assert _PathReader(target).tail(0, len(content), 1) == content


def test_path_tail_empty_file_returns_empty_bytes(tmp_path: Path) -> None:
    """An empty file should yield an empty tail sample, not an error."""
    target = tmp_path / "empty.csv"
    target.write_bytes(b"")

    assert _PathReader(target).tail(0, 4096, 1) == b""


def test_path_tail_missing_file_raises_domain_error(tmp_path: Path) -> None:
    """A missing source file should raise the domain-specific exception."""
    missing = tmp_path / "does_not_exist.csv"

    with pytest.raises(FileSampleReadError):
        _PathReader(missing).tail(0, 4096, 1)


def test_path_tail_never_reads_the_whole_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    tail = _PathReader(target).tail(0, len(marker), 1)

    assert tail == marker
    assert read_calls == [len(marker)]


def test_path_tail_on_real_sample_fixture_ends_with_last_row() -> None:
    """The tail of the bundled sample.csv should contain its final row."""
    tail = _PathReader(SAMPLE_CSV_PATH).tail(0, 200, 1)
...
def test_path_head_rejects_non_positive_budget(tmp_path: Path, n_bytes: int) -> None:
    """A non-positive head budget must be rejected: ``read(-1)`` reads the whole file."""
    target = tmp_path / "data.csv"
    target.write_bytes(b"a,b,c\n")

    with pytest.raises(ValueError, match="n_bytes"):
        sample_source(target, n_bytes, 0)


def test_path_tail_rejects_negative_budget(tmp_path: Path) -> None:
    """A negative tail budget must be rejected rather than silently misread."""
    target = tmp_path / "data.csv"
    target.write_bytes(b"a,b,c\n")

    with pytest.raises(ValueError, match="tail_bytes"):
        sample_source(target, 4, -1)


def test_path_tail_zero_budget_returns_empty_bytes(tmp_path: Path) -> None:
    """A zero tail budget is valid and yields an empty sample."""
    target = tmp_path / "data.csv"
    target.write_bytes(b"a,b,c\n")

    assert _PathReader(target).tail(0, 0, 1) == b""
