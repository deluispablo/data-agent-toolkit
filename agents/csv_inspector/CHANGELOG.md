...
- The README is reorganized to show what the library does before how it
  ...
  recording of a real session, the 0.3.0 accuracy baseline at a glance, a
  pandas recipe next to the quickstart, a four-step overview of the
  pipeline (the full diagram stays, folded), a side-by-side backend table,
  and the API, settings, CLI and errors grouped under "Reference". No
  content was dropped. The picture, its two demo files and the recording are
  generated by `scripts/render_readme_hero.py` and `docs/assets/demo.tape`.
