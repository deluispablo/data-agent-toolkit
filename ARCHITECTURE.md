...
| `_models.py` | `ColumnType`, `ColumnSchema`, `CSVInspectionResult` and `Usage` (attached as `result.usage`, excluded from dumps and the JSON Schema), with the validators that normalize small-model spellings (tab, "no escape", "no quoting", `null`/`-1` header index, type aliases). |
